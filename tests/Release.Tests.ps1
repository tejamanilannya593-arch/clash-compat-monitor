...
$required = @(
    ...
    'assets\social-preview.png','docs\images\service-incident-flow.png','docs\release-notes\v0.7.0-preview.7.md'
)
...
if (!$package.Contains('ClashCompatibilityMonitor-v0.7.0-preview.7') -or
    !$program.Contains('Version = "0.7.0-preview.7"')) { throw 'Release version is inconsistent.' }
...
if (!$install.Contains("Version='0.7.0-preview.7'") -or !$install.Contains('LegacyBrowserCompanionRemoved=$true') -or
...
if ($version.FileVersion -ne '0.7.0.0' -or $version.ProductVersion -ne '0.7.0-preview.7') { throw 'Windows version metadata is incorrect.' }
