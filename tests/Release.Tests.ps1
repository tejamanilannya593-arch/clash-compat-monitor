...
$required = @(
    ...
    'docs\release-notes\v0.6.3-preview.5.md'
)
...
if (!$packageScript.Contains('ClashCompatibilityMonitor-v0.6.3-preview.5')) { throw 'Release package version is not v0.6.3-preview.5.' }
...
if (!$installScript.Contains("Version='0.6.3-preview.5'")) { throw 'Installer status version is not v0.6.3-preview.5.' }
...
foreach ($exeName in @('ClashCompatibilityMonitor.exe','ClashCompatibilityMonitor.BrowserHost.exe')) {
    ...
    if ($version.FileVersion -ne '0.6.3.0' -or $version.ProductVersion -ne '0.6.3-preview.5') {
...
if (!$program.Contains('Version = "0.6.3-preview.5"') -or
