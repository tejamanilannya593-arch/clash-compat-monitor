...
try {
    ...
    [pscustomobject]@{Version='0.7.0-preview.5';ProcessId=$running[0].ProcessId;Backup=$backup;ClashFilesUnchanged=$true;LegacyBrowserCompanionRemoved=$true} | ConvertTo-Json -Compress
} catch {
    Stop-InstalledMonitor
    Stop-LegacyHost
    foreach ($registration in $registrations) {
