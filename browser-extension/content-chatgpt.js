(function (root, factory) {
  "use strict";
  const adapter = typeof module === "object" && module.exports ? require("./chatgpt-adapter") : root.CCMChatGptAdapter;
  const api = factory(adapter);
  if (typeof module === "object" && module.exports) module.exports = api;
  else {
    root.CCMChatGptContent = api;
  ...
  function validTask(task) {
    return task && /^[0-9a-f]{32}$/.test(task.taskId || "") && task.service === "ChatGPT" &&
      /^CCM-[A-Za-z0-9-]{1,76}$/.test(task.challenge || "") &&
      typeof task.prompt === "string" && task.prompt.length > 0 && task.prompt.length <= 300;
  }

  function createChatGptContentHandler(dependencies) {
    ...
    return function (message, sender) {
      if (!message || message.type !== "ccm-run" || !sender || sender.id !== runtime.id ||
          !validTask(message.task) || !location || location.origin !== "https://chatgpt.com") return false;
  ...
  function install(chromeApi, page, location) {
    chromeApi.runtime.onMessage.addListener(createChatGptContentHandler({
      runtime: chromeApi.runtime,
      page,
      location,
      adapter: adapterApi.runChatGptAdapter
    }));
  }

  return { createChatGptContentHandler, install, validTask };
});
