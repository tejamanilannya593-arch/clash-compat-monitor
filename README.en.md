...
- The local v0.6.3-preview.6 build aligns the ordinary proxy selector with a verified stable node and compares Google and GitHub reachability site-by-site across the probe and system proxy paths. Any mismatch identifies the site and path, then pauses automatic switching. The isolated probe, direct, and reject groups remain unchanged; Clash configuration files are not edited. This improves routing consistency but cannot guarantee website sign-in. The optional web entrance check remains off by default and does not verify sign-in, search, or downloads. This is not a formal GitHub release.
