...
- The local v0.7.0-preview.1 build uses Mihomo's runtime proxy types to exclude built-in actions and nested selectors without guessing from provider names; if types cannot be read, it falls back to the existing catalog. It no longer hot-reloads the complete Mihomo configuration. Unexpected runtime IPv6 pauses the cycle and asks the user to reapply the enhancement script. Ordinary-selector alignment and site-specific proxy-path diagnostics remain. Automatic restoration after a configuration reload is suspended in this preview; manual restoration still rechecks the target. This cannot guarantee website sign-in and is not a formal GitHub release.
