...
$release = Join-Path $distRoot 'ClashCompatibilityMonitor-v0.7.0-preview.10'
...
New-Item -ItemType Directory -Force -Path (Join-Path $release 'scripts'),(Join-Path $release 'clash'),(Join-Path $release 'docs\images'),(Join-Path $release 'docs\release-notes') | Out-Null
...
Copy-Item -LiteralPath (Join-Path $root 'docs\release-notes\v0.7.0-preview.10.md') -Destination (Join-Path $release 'docs\release-notes')
$zip = $release + '.zip'
